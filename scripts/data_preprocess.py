...
class Config:
    annotation_file   = '/data/image/ai_cha/caption/ai_challenger_caption_train_20170902/caption_train_annotations_20170902.json'
    val_annotation_file = '/data/image/ai_cha/caption/ai_challenger_caption_validation_20170910/caption_validation_annotations_20170910.json'
    unknown = '</UNKNOWN>'
    end='</EOS>'
    padding='</PAD>'
    max_words=10000
...
def process(**kwargs):
    ...
    train_data = {'caption':ix_captions,
                  'ix2id':ix2id,
                  'id2ix':id2ix
                    }


    with open(opt.val_annotation_file) as f:
        data = json.load(f)
    
    # 8f00f3d0f1008e085ab660e70dffced16a8259f6.jpg -> 0
    val_id2ix = {item['image_id']:ix for ix,item in enumerate(data) }
    # 0-> 8f00f3d0f1008e085ab660e70dffced16a8259f6.jpg
    val_ix2id = {ix:id for id,ix in (val_id2ix.items())}
    assert val_id2ix[val_ix2id[10]] == 10

    val_captions = [item['caption'] for item in data]
    # 分词结果
    val_cut_captions = [ [list(jieba.cut(ii,cut_all=False)) for ii in item ] for item in tqdm.tqdm(val_captions) ]

   

    val_ix_captions =[ [  [ word2ix.get(word,word2ix.get(opt.unknown)) for word in sentence] 
                        for sentence in item]
                        for item in val_cut_captions]

    val_data = {
        'caption':val_ix_captions,
        'ix2id':val_ix2id,
        'id2ix':val_id2ix
    }
...
def test2(**kwargs):
    class Dataset:
        def __init__(self):
            self.imgs = t.load('/data/image/ai_cha/caption/ai_challenger_caption_test1_20170923/test_imgs.pth')
